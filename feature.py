...
class ImageFeature(object):
    def __init__(self, image, params):
        ...
        if(params.deep):
            self.extractor = params.extractor
        else:
    ...
    def extract(self):
        if(self.deep):
            self.keypoints, _, self.descriptors = self.extractor.compute(self.image)
        else:
    ...
    def circular_stereo_match(self, *args, **kwargs):
        return circular_stereo_match(self.deep, self.matcher, *args, **kwargs)
...
def circular_stereo_match(
    ...
    if deep:
        ms13 = matcher.flann.knnMatch(np.array(desps1), np.array(desps3), k=2)
    else:
    ...
    if deep:
        ms24 = matcher.flann.knnMatch(np.array(desps2), np.array(desps4), k=2)
    else:
