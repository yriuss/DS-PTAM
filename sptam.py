...
class SPTAM(object):
    ...
    def write_to_file(self, file_path):
        ...
        np.savetxt(a_file, self.saved_poses, delimiter=' ', fmt='%.6e')

        a_file.close()
    def save_poses(self, frame):
        count = 0
        poses = [0]*12
        for i in range(3):
            for j in range(4):
                if(j < 3 and i < 3):
                    poses[count] = frame.pose.R[i][j]
                elif(j == 3):
                    poses[count] = frame.pose.t[i]
                count+=1
        self.saved_poses.append(np.array(poses))
...
if __name__ == '__main__':
    ...
    if params.deep:
        sptam.write_to_file(os.getcwd() + "/results/mine/" + args.path[-2:]+ ".txt")
    else:
        sptam.write_to_file(os.getcwd() + "/results/sptam/" + args.path[-2:]+ ".txt")
    sptam.stop()
    if visualize:
        viewer.stop()